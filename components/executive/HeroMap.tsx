...
const LAYERS = [
  { key: 'parcels',      label: 'Parcels',                  count: '2.04M', color: '#00e0ff', active: true  },
...
export default function HeroMap() {
  ...
  useEffect(() => {
    ...
    if (tileRef.current.labels) map.removeLayer(tileRef.current.labels);
  ...
  return (
    <div className="glass" style={{
      ...
      <div style={{ flex: 1, position: 'relative', minHeight: 0 }}>
        <div ref={mapRef} style={{ position: 'absolute', inset: 0, background: 'var(--bg-canvas)' }} />
        ...
        >
          <div className="eyebrow" style={{ padding: '4px 12px 8px' }}>Map Layers</div>
          {LAYERS.map(layer => {
